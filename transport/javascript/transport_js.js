...
if((class_id==0))
    {
      ...
      student_search(class_id,"0");   
       document.getElementById("section_name").innerHTML="<option value='0'>---Select---</option>"
       return false;
    }
try
  {
...
httpxml.send(null);
   ...
   {
   student_search(class_id,section_id);     
   }
   }
                 
//section to fetch student
                 
function student_search(class_id,section_id)
 {   
...
url=url+"?get_student_data=all&student_of_class_id="+class_id+"&student_of_section_id="+section_id;
httpxml.onreadystatechange=stateChanged;
httpxml.open("GET",url,true);
httpxml.send(null);
                 }
...
$result=mysql_query("SHOW TABLE STATUS LIKE 'category_db'");
$row=mysql_fetch_array($result);
$nextId=$row['Auto_increment']; 
$final_category_id="CATGRY_$nextId"; 
$encrypt_id=md5(md5($final_category_id));
      ...
      if((!empty($category_name)))
      {
       $check_category_record=  mysql_query("SELECT * FROM category_db WHERE organization_id='$fetch_school_id'
               and branch_id='$fetch_branch_id' and category_id='$final_category_id' and is_delete='none'
           OR organization_id='$fetch_school_id' and branch_id='$fetch_branch_id' and category_name='$category_name' and is_delete='none'");
      ...
      if((empty($fetch_category_data))&&($fetch_category_data==null)&&($fetch_category_num_rows==0))
      {
          ...
          $insert_category_db=  mysql_query("INSERT into category_db values('','$fetch_school_id','$fetch_branch_id'
                  ,'$final_category_id','$encrypt_id','$category_name','$category_full_name','$description'
       ...
       <?php  include_once '../header/header_page.php'; ?>   
        ...
        <div id="main_work_first_div">
            <div id="middel_work_div">
              ...
              <div id="transport_function" class="Short_menu_show"><b>Manage Category</b></div></div>
                
               <div class="main_work_data" style="">
                   <div class="left_add_work_div">
                    <div class="heading_title_td">Add New Category</div>    
                  ...
                  <table cellspacing="2" cellpadding="2" id="org_table_style" style=" font-size:12px;  float:left; ">
                   ...
                   <tr><td>Category Name <sup>*</sup></td>
                        <td><b>:</b></td>
                   ...
                   <div class="heading_title_td">View Category Details</div>    
                   <table cellspacing="0" cellpadding="0" class="session_fetch_details">
                     ...
                     {
                         $row++;
                                 $fetch_category_db_id=$fetch_category_data['id'];
                                 $fetch_category_unique_id=$fetch_category_data['category_id'];
                                 $encrypt_id=$fetch_category_data['encrypt_id'];
                                 $fetch_category_name=$fetch_category_data['category_name'];
                                 $fetch_category_full_name=$fetch_category_data['category_full_name'];
                                 $fetch_category_description=$fetch_category_data['description'];
                                 
                     echo "<tr id='delete_row_$fetch_category_unique_id' class='td_data_style'>
                                   <td><b>$row</b></td> 
                                       <td><b>$fetch_category_name</b></td>
                                       <td style='width:28%;'>$fetch_category_full_name</td>
                                       <td>$fetch_category_description</td>
                                <td style='border-right:1px solid gray; width:130px;'>";
                                {
                                    ?>
                        <abbr title="Edit Session Details">
                            <a style="color:blue;" href="#" onclick="window.open('edit_category_details.php?token_id=<?php  echo $encrypt_id;?>','size',config='height=400,width=580,position=absolute,left=300,top=0, addressbar=yes, location=no, toolbar=no, menubar=no,copyhistory=no, scrollbars=yes, resizable=yes, directories=no, status=no');  popup.focus();">  
                        <div class="edit_delete_buttons" style="background-color:green; width:45px;">Edit</div></a>
                        </abbr>
                        
                        
                        <abbr title="Delete Session">                            
                            <div onclick="delete_data('<?php  echo $fetch_category_unique_id;?>','category_delete_command')" class="edit_delete_button">Delete</div>
                       </abbr>

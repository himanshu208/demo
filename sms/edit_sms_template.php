...
$final_sms_message_id=$_POST['update_unique_id'];
 ...
 if((!empty($message_type))&&(!empty($template_name))&&(!empty($template))&&(!empty($final_sms_message_id)))
 {
  ...
  $select_db=mysql_query("SELECT * FROM sms_template_db WHERE $db_main_details_whout_session sms_template_id!='$final_sms_message_id' and message_type_id='$message_type' "
          . "and template_name='$template_name' and sms_template='$template'");  
  $select_num_rows=mysql_num_rows($select_db);
  if($select_num_rows==0)
  {
  
  $update_db=mysql_query("UPDATE sms_template_db SET message_type_id='$message_type',template_name='$template_name'"
      ...
      <?php  include_once '../ajax_loader_page_second.php';?>
       ...
       <form action="" method="post" name="form1" onsubmit="return validate_form();" enctype="multipart/form-data">
        ...
        <div class="edit_first_div">
            ...
            $select_data=mysql_query("SELECT * FROM sms_template_db WHERE $db_main_details encrypt_id='$token_id' and is_delete='none'"); 

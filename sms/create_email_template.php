...
$result=mysql_query("SHOW TABLE STATUS LIKE 'sms_email_template_db'");
$row=mysql_fetch_array($result);
$nextId=$row['Auto_increment']; 
$final_sms_email_message_id="MAIL_TMP_$nextId"; 
$encrypt_id=md5(md5($final_sms_email_message_id)); 
 ...
 if((!empty($message_type))&&(!empty($template_name))&&(!empty($template))&&(!empty($final_sms_email_message_id)))
 {
    
  $select_db=mysql_query("SELECT * FROM sms_email_template_db WHERE $db_main_details_whout_session message_type_id='$message_type' "
          . " and template_name='$template_name' and subject='$subject' and sms_template='$template' and is_delete='none'");  
  $select_num_rows=mysql_num_rows($select_db);
  if($select_num_rows==0)
  {
  $insert_db=  mysql_query("INSERT into sms_email_template_db values('','$fetch_school_id','$fetch_branch_id','$insert_session'"
          . ",'$final_sms_email_message_id','$encrypt_id','$message_type','$template_name','$subject','$template','none','$date','$date','$date_time')");  
                ...
                <div class="forward_step_style" style=" float:left; height:40px;  ">
                   <table cellspacing="0" cellpadding="0" class="forward_page_style">
                       <tr>
                           ...
                           <td>/</td>
                           <td><a href="sms_email_dashboard.php">SMS (Phone/Email)</a></td>
                           <td>/</td>
                           <td><a href="manage_template.php">Manage Template</a></td>
                           <td>/</td>
                           <td><a href="sms_email_template.php">SMS Template</a></td>
                           <td>/</td>
                  ...
                  <table cellsapcing="4" cellpadding="4" class="table_middle">
                       ...
                       <td>
                           <select class="select_class" id="message_type" name="message_type">
                             ...
                             <?php 
 $sms_email_message_type_db=mysql_query("SELECT * FROM sms_message_type_db WHERE $db_main_details_whout_session is_delete='none'");   
                     while ($fetch_sms_email_message_type_data=mysql_fetch_array($sms_email_message_type_db))
                     {
                       
                                 $fetch_sms_email_message_type_db_id=$fetch_sms_email_message_type_data['id'];
                                 $fetch_sms_email_message_type_unique_id=$fetch_sms_email_message_type_data['message_type_id'];
                                 $encrypt_id=$fetch_sms_email_message_type_data['encrypt_id'];
                                 $fetch_sms_email_message_type_name=$fetch_sms_email_message_type_data['message_type'];
                                $fetch_sms_email_message_type_description=$fetch_sms_email_message_type_data['description'];
                                echo "<option value='$fetch_sms_email_message_type_unique_id'>$fetch_sms_email_message_type_name</option>";
...
Code for student ( Studnet Name :#name# , Course : #course# , Batch : #batch# ).
                       ...
                       <td><input type="text" id="subject" name="subject" 
                                  placeholder="Enter Subject" class="text_box_class">
                          </td>
                       </tr>
                        <tr>
                       <td>Template</td>

...
var url="fee_payment_reconcile_ajax_code.php";
...
//onchange status
function status_change(status_name,recipt_id)
{
  
    if(status_name=="cleared")
   {
   document.getElementById("color_select_"+recipt_id).style.backgroundColor="green";   
   }else
       if(status_name=="pending")
   {
       document.getElementById("color_select_"+recipt_id).style.backgroundColor="orange";
   }else
       if(status_name=="cancel")
   {
     document.getElementById("color_select_"+recipt_id).style.backgroundColor="red";  
   }
}


//update fee payment status
function change_fee_status(recipt_id)
{
    
var organization_id=document.getElementById("organization_id").value;
   var branch_id=document.getElementById("branch_id").value;
   var session_id=document.getElementById("insert_session_id").value;
   var status=document.getElementById("color_select_"+recipt_id).value;
   if((organization_id==0)&&(branch_id==0)&&(session_id==0)&&(recipt_id==0)&&(status==0))
   {
      alert("Please fill all fields.")
      return false;
   }else
   {
   var r=confirm("Are you sure you want to "+status+" fee payment \n\
( Receipt No. - "+recipt_id+")");
if (r==true)
  { 
    var httpxml;
try
...
function stateChanged() 
 ...
 if(httpxml.responseText!=0)
     ...
     var status_check=document.getElementById("color_select_"+recipt_id).value;
     if(status_check=="cancel")
     {
      document.getElementById("color_select_"+recipt_id).disabled=true;
      document.getElementById("update_button_"+recipt_id).style.display="none";
     }
 ...
 {
  document.getElementById("ajax_loader_show").style.display="none";
         alert("Request failed,Please try agian later");
         return false;    
 }
...
var url="fee_payment_reconcile_ajax_code.php";
url=url+"?org_id="+organization_id+"&&branch_id="+branch_id+"&&session_id="+session_id+"&&status="+status+"&&fee_payment_id="+recipt_id;
httpxml.onreadystatechange=stateChanged;
httpxml.open("GET",url,true);
httpxml.send(null);     

...
$row=0;
    ...
    if($student_handicapped=="yes")
    {
     ...
     while ($fetch_fee_amount_data=mysql_fetch_array($fee_amount_dbs))
        ...
        echo "<tr><td>$fetch_fee_group_name</td></tr>";
...
$fee_category_discount_db=mysql_query("SELECT *,IF(financefeediscountcategory.discount_type='percantage',((feediscount*$fetch_fee_amount)/100),(feediscount)) as category_amount FROM financefeediscountcategory WHERE $db_main_details"
...
$category_discount_amount=$fetch_category_discount_data['category_amount'];
          
//particular student discount
$particular_student_discount=mysql_query("SELECT *,IF(financefeediscountparticularstudent.discount_type='percantage',((feediscount*$fetch_fee_amount)/100),(feediscount)) as par_student_amount FROM financefeediscountparticularstudent WHERE
...
$particluar_student_discount_amount=$fetch_student_fee_discount_data['par_student_amount'];

//student handicapped discount

 $student_handicapped_db=mysql_query("SELECT *,IF(financefeediscountstudenthandicapped.discount_type='percantage',((feediscount*$fetch_fee_amount)/100),(feediscount)) as handecaped_amount FROM financefeediscountstudenthandicapped WHERE $db_main_details"
...
$fetch_student_handicapped_data=mysql_fetch_array($student_handicapped_db);
 $student_handicapped_discount_amount=$fetch_student_handicapped_data['handecaped_amount'];      
...
$print_total_amount +=$final_payable_fees_student;
         ...
         }else
         {
           ...
           $start_check="";     
                 ...
                 $due_amount_db=mysql_query("SELECT *,SUM(due_amount) as total_due_amt FROM finance_student_due_amount_db WHERE student_id='$student_id'
                               and is_delete='none' and status='pending' and action='active'");               
                 $fetch_due_fee_amount_db=mysql_fetch_array($due_amount_db);
                 $total_both_fine_or_due_amount=$fetch_due_fee_amount_db['total_due_amt'];  
                              
                              
                  //final sub total amount
